...
const ChoiceBtn = styled.button<{ $active: boolean }>`
  ...
  background: ${(p) => (p.$active ? theme.colors.primary : theme.colors.white)};
  color: ${(p) => (p.$active ? theme.colors.white : theme.colors.textPrimary)};
  font-size: ${theme.fontSizes.sm};
  font-weight: ${theme.fontWeights.medium};
  cursor: pointer;
  font-family: ${theme.fonts.primary};
  &:hover {
    background: ${(p) => (p.$active ? theme.colors.primaryDark : theme.colors.gray50)};
  }
  &:focus-visible {
